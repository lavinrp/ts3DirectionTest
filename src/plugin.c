...
int ts3plugin_onTextMessageEvent(uint64 serverConnectionHandlerID, anyID targetMode, anyID toID, anyID fromID, const char* fromName, const char* fromUniqueIdentifier, const char* message, int ffIgnored) {
	...
	if(fromID == myID && fromID != toID) {  /* Don't reply when source is own client */
		if (!strcmp(message, "say something")) {
			run = 1;
			targetID = toID;

			//USE THIS FOR DEBUG IF YOU NEED IT
			if(ts3Functions.requestSendPrivateTextMsg(serverConnectionHandlerID, "run", fromID, NULL) != ERROR_ok) {
...
void ts3plugin_onEditPlaybackVoiceDataEvent(uint64 serverConnectionHandlerID, anyID clientID, short* samples, int sampleCount, int channels) {
	if (run && clientID == targetID) {
		float directTalkingVolume = 0.01f;
		for (int i = 0; i < sampleCount * channels; i++) {
			samples[i] = (short)(samples[i] * directTalkingVolume);
		}
	}
}
